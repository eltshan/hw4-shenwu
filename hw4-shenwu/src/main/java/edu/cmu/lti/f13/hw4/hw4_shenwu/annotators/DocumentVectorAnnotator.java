...
public class DocumentVectorAnnotator extends JCasAnnotator_ImplBase {
  ...
  public void process(JCas jcas) throws AnalysisEngineProcessException {
    ...
    while (DocumentIter.hasNext()) {
      ...
      for (int i = 0; i < doc.getTokenArray().size(); i++) {
    ...
    while (DocumentIter.hasNext()) { // for each document
      Document doc = (Document) DocumentIter.next();
      if (doc.getQueryID() != currentID) {
        currentID++;
      ...
      for (int j = 0; j < doc.getTokenArray().size(); j++) {
        // currentHashMap.get(doc.getText())

        int currentIndex = (Integer) currentHashMap.get(doc.getTokenArray(j).getText());
        
       // intArray.set(currentIndex,
       //         intArray.get((Integer) currentHashMap.get(doc.getTokenArray(j).getText())) + 1);
       intArray.set(currentIndex,
                doc.getTokenArray(j).getFrequency());
      }
      //
      // intArray.set( , v);
  ...
  private void createTermFreqVector(JCas jcas, Document doc) {
    ...
    while (tokenizer.hasNext()) {
      ...
      if (hash.containsKey(temp.word())) {
        ...
        tmpToken.setFrequency(tmpToken.getFrequency() + 1);
        // doc.set
      } else {
